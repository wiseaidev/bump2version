...
use bump2version::version::{
    BumpPart, bump_version, extract_format_keys, parse_version, render_format, serialize_version,
};
...
fn test_bump_patch() {
    let cfg = default_cfg();
    let v = parse_version("1.2.3", &cfg).unwrap();
    let bumped = bump_version(&v, &BumpPart::Patch, &cfg).unwrap();
...
fn test_bump_major_resets_minor_and_patch() {
    ...
    let bumped = bump_version(&v, &BumpPart::Major, &cfg).unwrap();
...
fn test_bump_unknown_component_returns_error() {
    let cfg = default_cfg();
    let v = parse_version("1.2.3", &cfg).unwrap();
    let result = bump_version(&v, &BumpPart::Custom("nonexistent".to_string()), &cfg);
...
fn test_cyclic_stage_bump() {
    ...
    let bumped = bump_version(&v, &BumpPart::Custom("stage".to_string()), &cfg).unwrap();
...
fn test_full_bump_and_serialize_roundtrip() {
    ...
    for (input, part, expected) in &[
        ("1.0.0", BumpPart::Patch, "1.0.1"),
        ("1.0.9", BumpPart::Patch, "1.0.10"),
        ("1.9.9", BumpPart::Minor, "1.10.0"),
        ("9.9.9", BumpPart::Major, "10.0.0"),
    ] {
